    ...
    <div>
        <Dialog class="min-w-1/2">
            ...
            <DialogContent>
                ...
                <ScrollArea class="h-[300px] w-full p-2 lg:h-[450px]">
                    <div class="grid grid-cols-2 gap-x-2 gap-y-2 text-xs lg:text-sm">
                        ...
                        <SchoolDetailRow :data="schoolDetails.schulform?.split('|').join(',<br>')"
                                         :title="'Schulformen:'"/>
                        <SchoolDetailRow :data="schoolDetails.ganztagsform" :title="'Ganztagsform:'"/>
                        <SchoolDetailRow :data="schoolDetails.abschluss?.split('|').join(',<br>')"
                                         :title="'Abschlüsse:'"/>
                        <SchoolDetailRow :data="schoolDetails.fremdsprache?.split('|').join(',<br>')"
                                         :title="'Fremdsprachen:'"/>
