...
<template>
    <Pagination :items-per-page="schoolPaginationItems.per_page" :total="schoolPaginationItems.total" class="pt-4">
        <PaginationContent>
            <Link
                :disabled="schoolPaginationItems.current_page === 1"
                :href="schoolPaginationItems.first_page_url"
                method="get"
                preserve-scroll
                preserve-state
            >
            ...
            <Link
                :disabled="schoolPaginationItems.current_page === 1"
                :href="schoolPaginationItems.prev_page_url ?? ''"
                method="get"
                preserve-scroll
                preserve-state
            >
            ...
            <template v-for="(item, index) in schoolPaginationItems.links.slice(1, -1)" :key="index">
                <PaginationItem v-if="item.url" :is-active="item.active" :value="index" class="hidden lg:block">
                    <Link :href="item.url ?? '#'" method="get" preserve-scroll preserve-state>
                        {{ item.label }}
                    </Link>
                </PaginationItem>
